...
> 作成日: 2026-02-14
> 更新日: 2026-05-21
> 関連: [vision.ja.md](vision.ja.md), [spec.ja.md](spec.ja.md), [specs/20260214_priming-layer_design.md](specs/20260214_priming-layer_design.md)
...
| **E: 未完了タスク** | TaskBoard + task_queue + 並列タスク | 500 | TaskBoard projection、失敗時はTaskQueueManagerへフォールバック | 「やるべきこと」 |
| **F: エピソード** | episodes/ | 800 | ベクトル検索（RAG）+ ポインタ整形 | 過去行動の意味的検索 |
| **G: グラフ文脈** | MemoryBackend | 500 | community context + recent facts | 多ホップ文脈・最近の事実 |
...
長期インデックス対象（`knowledge`, `episodes`, `procedures`, `common_knowledge`）は、下記の密ベクトル検索と任意のグラフ拡散で取得する。**`search_memory` には次の拡張がある:**

- **`activity_log` スコープ:** 統一アクティビティログ JSONL の**直近3日間**を対象とする **BM25 キーワード検索**（ベクトル類似度ではない）。
- **`all` スコープ:** ベクトル検索の結果と `activity_log` の BM25 結果を **RRF（Reciprocal Rank Fusion, k=60）** で統合する。

これにより、長期記憶にまだ固定化されていない**直近のツール結果**（メール本文・メッセージ断片など）も想起しやすくなる。

メインコーパスでは当初 BM25 とベクトルのハイブリッドを検討したが、多言語では密ベクトル検索単体の方が有利なため、各スコープの RAG は**ベクトル優先**としている。キーワード検索は、鮮度や語彙一致が重要な **`activity_log` の短期ウィンドウ**に限定する。

### バックエンド方針

`legacy` は AnimaWorks の安定版・既定 memory backend である。`search_memory`、priming、consolidation、通常の Anima 運用では、この経路を本番用として扱う。

`neo4j` は現時点では experimental な opt-in backend である。グラフ記憶の研究、ローカル評価、特定 Anima での限定実験に使う。利用する場合は、`animaworks anima set-memory-backend <name> neo4j` または `animaworks memory migrate --to neo4j --activate-global` により、運用者が実験的挙動を明示的に受け入れる。通常の migration はグラフデータを準備するだけで、global default backend は変更しない。
...
| `vector_worker_fallback_direct` | `false` | ワーカー不能時に同一プロセス内ChromaDBへ戻すか。既定は戻さない |
| `repair_enabled` | `true` | RAG不整合の検出・修復を有効化 |
...
`procedures/` は長期記憶としての手順書である。一方、`skills/` は実行時に呼び出しやすい形へ整理された手順・能力のカタログであり、`core/skills/` が管理する。現在の priming はスキルを自動注入せず、手順・スキルの提示は以下の経路で行う。

| 経路 | 実装 | 説明 |
|---|---|---|
| 明示的 activation | `core/skills/activation.py` | セッションやスレッドに対して使うスキルを明示し、本文またはポインタをプロンプトに入れる |
| Skill Router | `core/skills/router.py` | カタログメタデータから候補スキルを選び、routing 評価や候補提示に使う |
| Skill Hub | `core/skills/hub.py` | 共通・個人スキルの install / list / inspect / remove / quarantine を扱う |
| Curator | `core/skills/curator.py` | 使用状況、失敗率、古さ、メタデータ不足を見て review / archive / merge などを提案する |
| Promotion | `core/skills/promotion.py` | 実績のある `procedures/` を quarantine skill または probation skill として昇格する |
| Autolearn | `core/skills/autolearn.py` | 低リスクで条件を満たした手順を、自律的に probation skill へ変換する |

`core/memory/skill_metadata.py` には description ベースのマッチング関数が残っており、互換性や一部ユーティリティで使える。ただし、現在の自動想起はスキルを直接注入する設計ではない。必要な時は active skill context、`read_memory_file`、または Skill Hub のカタログを経由して読み込む。
...
| `priming/gate.py` | deterministic priming gate。表示可否と render mode を決める |
...
| `priming/channel_a.py` 〜 `channel_c.py` | 送信者、活動、重要知識・関連知識 |
| `priming/channel_e.py` | TaskBoard / task_queue 由来の保留タスク |
...
| `action_gate.py` | `ActionMemoryGate` | 外部送信・記憶書き込みなど副作用のある行動前の記憶確認 |
...
| `taskboard_housekeeping.py` | — | TaskBoard の stale processing / deferred / suppressed / background 状態の整理 |
...
| `housekeeping.py` | `run_housekeeping()` | ログ・shortterm・TaskBoard・Skill Curator 等の日次クリーンアップ |
...
| `rag/singleton.py` | — | ベクトルストア・埋め込みのプロセス内シングルトン。通常プロセスからの直接ChromaDBアクセスを抑制 |
| `rag/vector_worker.py` / `rag/vector_worker_client.py` | — | ChromaDB / sentence-transformers を隔離ワーカーとして起動・プロキシする |
| `rag/repair_service.py` | `RAGRepairService` | ChromaDB不整合を検出し、quarantine + full reindex を制御する |
| `rag/repair_rebuild.py` | — | `vectordb` の隔離とフル再構築 |
| `rag/watcher.py` | `FileWatcher` | ファイル変更監視と再インデックス |

### スキル（`core/skills/`）

記憶システムから独立したサブシステムだが、手続き記憶の昇格先として密接に関係する。

| モジュール | 役割 |
|---|---|
| `activation.py` | スレッド単位の active skill とプロンプト注入 |
| `router.py` | スキル候補のルーティング評価 |
| `hub.py` | Skill Hub の install / list / inspect / remove / quarantine |
| `curator.py` | 利用状況から review / archive / merge などを提案 |
| `promotion.py` / `probation_promotion.py` | `procedures/` から skill への昇格 |
| `autolearn.py` | 低リスク手順の probation skill 化 |
...
- [spec.ja.md](spec.ja.md) — 要件定義書（書庫型記憶の基本設計）
